$(document).ready(function () {
    ...
    function campoemail() {
        ...
        $.ajax({
            url: nameproject + '/services/TipoContato/BuscarNome/Email', //lugar onde a servlet está
            type: 'GET',
    ...
    function enchesexo() {
        //fazer a chamado da servlet
        $.ajax({
            url: nameproject + '/listarSexo', //lugar onde a servlet está
            type: 'POST',
            data: {
                oper: "1"
            },
    ...
    function enchetipodocumento() {
        //fazer a chamado da servlet
        $.ajax({
            url: nameproject + '/services/TipoDocumento/Listar', //lugar onde a servlet está
            type: 'GET',
            complete: function (e, xhr, result) {
                if (e.readyState == 4 && e.status == 200) {
                    try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    function enchetipocontato() {
        //fazer a chamado da servlet
        $.ajax({
            url: nameproject + '/services/TipoContato/Listar', //lugar onde a servlet está
            type: 'GET',
            complete: function (e, xhr, result) {
                if (e.readyState == 4 && e.status == 200) {
                    try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    function encheestadocivil() {
        //fazer a chamado da servlet
        $.ajax({
            url: nameproject + '/services/EstadoCivil/Listar', //lugar onde a servlet está
            type: 'GET',
            complete: function (e, xhr, result) {
                if (e.readyState == 4 && e.status == 200) {
                    try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    function encheescolaridade() {
        //fazer a chamado da servlet
        $.ajax({
            url: nameproject + '/services/Escolaridade/Listar', //lugar onde a servlet está
            type: 'GET',
            complete: function (e, xhr, result) {
                if (e.readyState == 4 && e.status == 200) {
                    try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    $("#inputBuscarProfissao").autocomplete({
        ...
        source: function (request, response) {
            $.ajax({
                url: nameproject + '/services/Profissao/BuscarNome/' + $("#inputBuscarProfissao").val(), //lugar onde a servlet está
                dataType: "json",
    ...
    function carregarDados() {
        ...
        if (dados !== "" && dados.length >= 2) {
            $.ajax({
                url: nameproject + '/services/Profissao/BuscarId/' + dados, //lugar onde a servlet está
                dataType: "json",
                success: function (data) {
                    ...
                    //desativa o campo input para não permitir a entrada
                    desabilita("inputBuscarProfissao");
                    $('#inputBuscarProfissao').val("Profissão selecionada");
...
//     Função para limpar os campos caso a busca esteja vazia
    ...
    $("select#cmbtipodocumento").change(function () {
        ...
        if (i == 0) {
            ...
            //fazer a chamado da servlet
            $.ajax({
                url: nameproject + '/services/TipoDocumento/Buscar/' + i, //lugar onde a servlet está
                type: 'GET',
                complete: function (e, xhr, result) {
                    if (e.readyState == 4 && e.status == 200) {
                        try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    $("select#cmbtipocontato").change(function () {
        ...
        if (i == 0) {
            ...
            //fazer a chamado da servlet
            $.ajax({
                url: nameproject + '/services/TipoContato/Buscar/' + i, //lugar onde a servlet está
                type: 'GET',
                complete: function (e, xhr, result) {
                    if (e.readyState == 4 && e.status == 200) {
                        try { //Converte a resposta HTTP JSON em um objeto JavaScript
    ...
    $('#verificaUsuario').click(function () {
        ...
        $.ajax({
            url: nameproject + '/services/Usuario/BuscarNome/' + username, //lugar onde a servlet está
            type: "GET",
            complete: function (e, xhr, result) {
                if (e.readyState == 4 && e.status == 200) {
                    try { //Converte a resposta HTTP JSON em um objeto JavaScript
                    ...
                    if (Obj != null) {
                        ...
                        if (String(Obj.login) !== username) {
                            ...
                            div.innerHTML = "Este usuário está disponível!";
                        } else {
                            div.style.display = "contents";
                            div.style.color = "red";
                            desabilita('inputPassword');
                            div.innerHTML = "Usuário já foi cadastrado! Tente outro!";
                        }
                    }
                }
            }
